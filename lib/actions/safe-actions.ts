    ...
    try {
      ...
      if (cachedData) {
        try {
          ...
          console.error(
            "Error parsing cached YouTube connection data:",
            parseError
          );
        }
      }
    ...
    try {
      ...
      if (cachedData) {
        try {
          ...
          // If parsing fails, continue to database query
          console.error(
            "Error parsing cached Google Drive connection data:",
            parseError
          );
        }
      }
      ...
      console.error(
        "Database error fetching Google Drive connection:",
        dbError
      );
      return { connected: false, error: "Database error", details: null };
    }
    ...
    try {
      ...
      if (cachedData) {
        try {
          ...
          // If parsing fails, continue to database query
          console.error(
            "Error parsing cached Dailymotion connection data:",
            parseError
          );
        }
      }
      ...
      console.log(
        "🔍 Dailymotion Connection Check - User connected:",
        isConnected
      );
      console.log(
        "🔍 Dailymotion Connection Check - User info available:",
        !!dailymotionInfo
      );
      ...
      const result = {
        ...
        details: isConnected
          ? {
              ...
              connectedAt:
                dailymotionInfo?.connectedAt?.toISOString() ||
