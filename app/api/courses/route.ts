...
export async function GET(request: Request) {
  try {
    ...
    const cacheKey = REDIS_KEYS.PUBLIC_COURSES(
      search,
      limit,
      offset,
      level,
    ...
    try {
      const [cachedCourses, cacheTimestamp] = await Promise.all([
        redis.get(cacheKey),
        redis.get(cacheTimestampKey),
      ]);
      ...
      if (cachedCourses && typeof cachedCourses === "string") {
        ...
        if (
          ...
          now - parseInt(cacheTimestamp) > SHORT_CACHE_DURATION * 1000
        ) {
...
async function fetchAndCacheCourses(
  ...
  const where: any = {
    isPublished: true,
    visibility: "PUBLIC", // Only show public courses, exclude hidden ones
  };
  ...
  const processedCourses: ProcessedCourse[] = courses.map(
    (course: DatabaseCourse) => {
      ...
      const processedCourse = {
        ...
        lectureCount: course.sections.reduce(
          (total, section) => total + section._count.lectures,
          0
        ),
  ...
  // Cache the results
  await Promise.all([
    redis.set(cacheKey, JSON.stringify(processedCourses), {
      ex: LONG_CACHE_DURATION,
    }),
    redis.set(cacheTimestampKey, Date.now().toString(), {
      ex: LONG_CACHE_DURATION,
    }),
  ]);
